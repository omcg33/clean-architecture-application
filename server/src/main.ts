...
import { createRender } from '@clean-arch/client';
...
async function bootstrap() {
  ...
  app.use(morgan('tiny'));
  app.useStaticAssets(ASSETS_PATH, {
    redirect: false,
    prefix: basePath,
  });
  ...
  app.listen(port, host, async () => {
    ...
    clientService.setPagesRoutes(filterPagesRoutes(namedRoutes));
    clientService.setApiRoutes(filterApiRoutes(namedRoutes));
    ...
    console.log(
      CONFIG.NODE_TLS_REJECT_UNAUTHORIZED,
      configService.get(CONFIG.NODE_TLS_REJECT_UNAUTHORIZED),
    );
  });
}
